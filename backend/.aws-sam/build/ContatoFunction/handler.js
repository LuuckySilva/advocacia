...
function emailAdvogado(data, advNome) {
    return `
    <html>
...
function emailCliente(data, advNome, advArea) {
    return `
    <html>
    <body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px;">
      <h2 style="color: #111; border-bottom: 2px solid #d4af6a; padding-bottom: 8px;">
...
const handler = async (event) => {
    ...
    // Validação
    const { valido, erros } = (0, validator_1.validarContato)(body);
    if (!valido) {
        return resposta(400, { success: false, message: erros.join(' | ') });
    }
    const data = body;
    const advogado = (0, router_1.rotearAdvogado)(data.area);
